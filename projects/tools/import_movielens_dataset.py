...
mlds_folder = '/home/yapianyu/Desktop/movielens/ml-10M100K/'
...
def get_movie_num_each_year():
    movie_num = {}
    f = open(mlds_folder + 'movies.dat')
    for line in f:
...
def import_to_mongodb():
    ...
    for line in f:
        ...
        movie_year = title[-5:-1]
        movie_title = title[:title.index('(')].decode('utf-8')
        mid2title[mid] = movie_title
        mid2year[mid] = movie_year

    f = open(mlds_folder + 'ratings.dat')
    for line in f:
        uid, mid, rating, _ = tuple(line.split('::'))
        email = 'ml' + str(uid) + '@movielens.org'
        movie_title = mid2title[mid]
        movie_year = mid2year[mid]
        ...
        if max_movie:
            mongodb['ratings'].update({'user_id': email2objid[email], 'movie_id': ObjectId(max_movie['_id'])},
                                      {'$set': {'rating': movie_rating, 'last_updated': datetime.datetime.utcnow()}},
                                      upsert=True)
            print max_score, uid, movie_year, mid, movie_rating

# import_72000_users()
# get_movie_num_each_year()
