                ...
                try:
                    ...
                    with open(zip_path, 'wb') as out:
                        while True:
                            ...
                            if len(data) == 0:
                                break
                            out.write(data)
                    self.logger.info('Downloaded <%s>' % zip_name)
                    # extract
                    zip_file = gzip.open(zip_path)
                    with open(xml_path, 'w') as out:
                        for line in zip_file:
                            out.write(line)
                    self.logger.info('Extracted <%s>' % xml_name)
                    # filter movie urls
                    for _, element in etree.iterparse(xml_path, tag='{http://www.sitemaps.org/schemas/sitemap/0.9}loc'):
                        ...
                        if match:
                            ...
                            low_priority_movie_pool.put(match.groupdict().get('id'))
                    ...
                    self.logger.error('Server cannot fulfill the request <%s> <%s> <%s>' % (zip_url, e.code, e.msg))
                    if e.code == 404:  # no more sitemap found
                        break
                except URLError, e:
                    self.logger.error('Failed to reach server <%s> <%s>' % (zip_url, e.reason))
                except Exception, e:
                    self.logger.error('%s <%s>' % (e, zip_url))

    def run(self):
        while True:  # infinite loop
            self.logger.info('==========InitialCrawler Started==========')
            ...
            while True:
                try:
                    ...
                    self.logger.debug('Crawled <%s>' % url_to_crawl)
                    ...
                    self.logger.error('Server cannot fulfill the request <%s> <%s> <%s>' % (url_to_crawl, e.code, e.msg))
                except URLError, e:
                    self.logger.error('Failed to reach server <%s> <%s>' % (url_to_crawl, e.reason))
                except Exception, e:
                    self.logger.error('%s <%s>' % (e, url_to_crawl))

            self.logger.info('==========InitialCrawler Finished==========')
...
class LowPriorityMovieFetcher(threading.Thread):
    ...
    def run(self):
        while True:  # infinite loop
            try:
                movie_id = low_priority_movie_pool.get()  # blocks if the queue is empty
                movie_info = get_movie_info(movie_id)
                subtype = movie_info.pop('subtype')
                if subtype == 'movie':
                    mongodb['movies'].update({'douban.id': movie_id}, {'$set': movie_info}, upsert=True)
                    self.logger.info('Crawled movie <%s>' % movie_info.get('title'))
                else:
                    mongodb['tv'].update({'douban.id': movie_id}, {'$set': movie_info}, upsert=True)
                    self.logger.info('Crawled tv <%s>' % movie_info.get('title'))

            except PyMongoError, e:
                self.logger.error('Mongodb error <%s>' % e)
            except HTTPError, e:
                self.logger.error('Server cannot fulfill the request <%s> <%s> <%s>' % (construct_movie_api_url(movie_id), e.code, e.msg))
            except URLError, e:
                self.logger.error('Failed to reach server <%s> <%s>' % (construct_movie_api_url(movie_id), e.reason))
            except Exception, e:
                self.logger.error('%s <%s>' % (e, construct_movie_api_url(movie_id)))

class StatusReporter(threading.Thread):
    def __init__(self):
        self.logger = logger.getChild('StatusReporter')
        threading.Thread.__init__(self)

    def run(self):
        while True:
            self.logger.debug('Totally Crawled=%s, Tag URL=%s, Movie URL=%s, Left Movie IDs=%s' % (len(crawled_url_pool), tag_url_pool.qsize(), movie_url_pool.qsize(), low_priority_movie_pool.qsize()))
            time.sleep(5)
