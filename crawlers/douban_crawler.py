    ...
    def __start_crawl(self):
        ...
        while self.__uncrawled_urls:
            try:
                ...
                # Is a movie page?
                if MOVIE_LINK_RE.match(url_to_crawl):
                    ...
                    DoubanCrawler.logger.info('Crawled movie #%s <%s %s> C(%s) U(%s)' %
                                              (self.__total_movies_crawled, movie_info['year'] if 'year' in movie_info else None, movie_info['titles'][0] if 'titles' in movie_info else None, len(self.__crawled_urls), len(self.__uncrawled_urls)))
