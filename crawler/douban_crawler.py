...
movie_link_re = re.compile('^http://movie\.douban\.com/subject/[0-9a-zA-Z]+/$')
...
class DoubanCrawler():
                ...
                for link_element in link_elements:
                    ...
                    if self.__url_is_allowed(url_in_page):
                        ...
                        if url_md5 not in self.__crawled_urls:
                            ...
                            # Validate movie link use predefined regex.
                            if movie_link_re.match(url_in_page):
                                ...
                                self.__get_movie_info(movie_id)

            except HTTPError, e:
                DoubanCrawler.logger.error('Server cannot fulfill the request <%s HTTP Error %s: %s>' % (url_to_crawl, e.code, e.msg))
            except URLError, e:
    ...
    def __get_movie_info(self, movie_id):
        try:
            ...
            DoubanCrawler.logger.info('Crawled movie #%s <%s %s>' % (self.__total_movies_crawled, movie_year, movie_title))
...
#                                   'http://movie.douban.com/nowplaying/beijing/',  # 正在上映
